  ...
  savePdf() {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this._form.typeExport === 'pdf') {
        ...
        var pdf = this._pdf.doc.output('dataurlstring'); // UMD support


        var versionPdfJS = ((_b = (_a = window) === null || _a === void 0 ? void 0 : _a.pdfjsLib) === null || _b === void 0 ? void 0 : _b.version) || version;
        GlobalWorkerOptions.workerSrc = "https://cdnjs.cloudflare.com/ajax/libs/pdf.js/".concat(versionPdfJS, "/pdf.worker.js");
        getDocument(pdf).promise.then(pdf => {
          pdf.getPage(1).then(page => {
            ...
            task.promise.then(() => {
              downloadURI(canvas.toDataURL("image/".concat(this._form.typeExport)), this._config.filename + ".".concat(this._form.typeExport));
              canvas.remove();
              resolve();
            });
          });
...
class SettingsModal {
  constructor(map, options, i18n, printMap) {
    ...
    this._modal.on('shown', () => {
      ...
      var actualScale = this._modal.el.querySelector('.actualScale');
  ...
  Footer(i18n, params) {
    ...
    return createElement("div", null, createElement("button", {
      ...
      id: "printTypeExport"
    }, mimeTypeExports.map(type => createElement("option", Object.assign({
      value: type.value
    }, type.selected ? {
      selected: 'selected'
...
function deepObjectAssign(target) {
  ...
  sources.forEach(source => {
    Object.keys(source).forEach(key => {
      ...
      target[key] = t_val && s_val && typeof t_val === 'object' && typeof s_val === 'object' && !Array.isArray(t_val) // Don't merge arrays
    ...
    setTimeout(() => {
      ...
      this._renderCompleteKey = this._map.once('rendercomplete', () => {
        domtoimage.toJpeg(this._mapTarget.querySelector('.ol-unselectable.ol-layers'), {
          ...
          if (this._isCanceled) return;
          yield this._pdf.savePdf(); // Reset original map size
