...
class Pdf {
    ...
    create(orientation, format, height, width) {
        // UMD support
        const _jsPDF = window.jspdf?.jsPDF || jsPDF;
        return {
    ...
    savePdf() {
        ...
        return new Promise((resolve, reject) => {
            if (this._form.typeExport === 'pdf') {
                ...
                // UMD support
                const versionPdfJS = window?.pdfjsLib
                    ?.version || version;
     ...
     * @protected
     */
    _calculateOffsetByPosition = (position, offset, size = 0) => {
        let x, y;
        switch (position) {
            case 'topleft':
     ...
     * @protected
     */
    _addRoundedBox = (x, y, width, height, bkcolor, brcolor) => {
        const rounding = 1;
     ...
     * @protected
     */
    _addText = (x, y, width, fontSize, color, align = 'left', str) => {
     ...
     * @protected
     */
    _addTextByOffset = (position, offset, width, fontSize, color, align, str) => {
        ...
        return (createElement("form", { id: "printMap" },
            createElement("div", null,
                createElement("div", { className: "printFieldHalf" },
                    ...
                    createElement("select", { name: "printFormat", id: "printFormat" }, paperSizes.map((paper) => (createElement("option", { value: paper.value, ...(paper.selected
                            ? { selected: 'selected' }
                            : {}) }, paper.value))))),
                createElement("div", { className: "printFieldHalf" },
            ...
            createElement("div", null,
                createElement("div", { className: "printFieldHalf" },
                    ...
                    createElement("select", { name: "printResolution", id: "printResolution" }, dpi.map((d) => (createElement("option", { value: d.value, ...(d.selected
                            ? { selected: 'selected' }
                            : {}) },
                        d.value,
                        " dpi"))))),
    ...
    Footer(i18n, options) {
        ...
        return (createElement("div", null,
            ...
            createElement("div", { class: "typeExportContainer" },
                ...
                createElement("select", { className: "typeExport", name: "printTypeExport", id: "printTypeExport" }, mimeTypeExports.map((type) => (createElement("option", { value: type.value, ...(type.selected
                        ? { selected: 'selected' }
...
class PdfPrinter extends Control {
    ...
    createPdf(options, showLoading) {
        options = {};
        this._printMap({
            format: (this._options.paperSizes.find((p) => p.selected) ||
                this._options.paperSizes[0]).value,
            resolution: (this._options.dpi.find((p) => p.selected) ||
