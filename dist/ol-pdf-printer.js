(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('ol/control/Control'), require('ol/proj'), require('jspdf'), require('ol/proj/Units')) :
  typeof define === 'function' && define.amd ? define(['ol/control/Control', 'ol/proj', 'jspdf', 'ol/proj/Units'], factory) :
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.PdfPrinter = factory(global.ol.control.Control, global.ol.proj, global.jsPDF, global.ol.proj.Units));
}(this, (function (Control, proj, jspdf, Units) { 'use strict';
  ...
  var require$$0 = /*@__PURE__*/getAugmentedNamespace(modal$1);
  ...
  var Content = function Content(i18n, params) {
    ...
    })))), mapElements.description && createElement("div", null, createElement("label", {
  ...
  var Footer = function Footer(lang) {
  ...
  var hidePrintModal = function hidePrintModal() {
    modal.hide();
  };
  var showPrintModal = function showPrintModal() {
  ...
  var ElementsPDF = function ElementsPDF() {
    var _this = this;
    ...
    this.addRoundedBox = function (x, y, width, height, bkcolor, brcolor) {
      var rounding = 1;
    ...
    this.addTextByOffset = function (position, offset, width, fontSize, color, align, str) {
      var _this$calculateOffset = _this.calculateOffsetByPosition(position, offset),
          x = _this$calculateOffset.x,
          y = _this$calculateOffset.y;
    ...
    this.addWatermark = function (watermark, position, offset, fontSize) {
      return new Promise(function (resolve) {
        var imageSize = 12;
        var fontSizeSubtitle = fontSize / 1.8;
        var back = false;

        var _this$calculateOffset3 = _this.calculateOffsetByPosition(position, offset),
            x = _this$calculateOffset3.x,
            y = _this$calculateOffset3.y;
        ...
        if (watermark.title) {
          ...
          _this.addRoundedBox(x - widthBack + 4 - acumulativeWidth, y - 4, widthBack + acumulativeWidth, height, '#ffffff', '#ffffff');
        ...
        if (watermark.subtitle) {
          ...
          if (!back) {
            ...
            _this.addRoundedBox(x - _widthBack + 3 - acumulativeWidth, y - 4, _widthBack + acumulativeWidth, 16, '#ffffff', '#ffffff');
        ...
        if (watermark.logo) {
          ...
          if (!back) {
            ...
            _this.addRoundedBox(x - _widthBack2 + 4, y - 4, _widthBack2, 16, '#ffffff', '#ffffff');
          }
    ...
    this.addDate = function (position, width, offset, fontSize, txcolor, align) {
      _this._pdf.doc.setFont('helvetica', 'normal');
    ...
    this.addUrl = function (position, width, offset, fontSize, txcolor, align) {
    ...
    this.addAttributions = function (position, width, offset, fontSize, txcolor, align) {
      _this._pdf.doc.setFont('helvetica', 'normal');
    ...
    this.addScaleBar = function (position, offset) {
      ...
      var porcentageMargin = _this._pdf.width / _this._style.margin;
      ...
      var x = offset.x + _this._style.margin;
      var y = offset.y + _this._style.margin; // if position is on the right, x needs to be calculate with pdf width and
      // the size of the element

      if (['topright', 'bottomright'].indexOf(position) !== -1) {
        x = _this._pdf.width - offset.x - fullSize - 8 - _this._style.margin * 2;
      }

      if (['bottomright', 'bottomleft'].indexOf(position) !== -1) {
        y = _this._pdf.height - offset.y - 10 - _this._style.margin;
      ...
      _this.addRoundedBox(x, y, fullSize + 8, 10, _this._style.bkcolor, _this._style.brcolor); // draw first part of scalebar
      ...
      _this._pdf.doc.text(scaleBarX, scaleBarY - 1, '0'); // /4 and could give 2.5. We still round, because of floating point arith


      _this._pdf.doc.text(scaleBarX + size - 1, scaleBarY - 1, (Math.round(length * 10 / 4) / 10).toString());

      _this._pdf.doc.text(scaleBarX + size * 2 - 2, scaleBarY - 1, Math.round(length / 2).toString());

      _this._pdf.doc.text(scaleBarX + size * 4 - 4, scaleBarY - 1, Math.round(length).toString() + ' ' + unit);
    };

    this.addCompass = function (imgSrc, position, offset, size, rotationRadians) {
      return new Promise(function (resolve) {
        var imageSize = 100;

        var _this$calculateOffset4 = _this.calculateOffsetByPosition(position, offset, size),
            x = _this$calculateOffset4.x,
            y = _this$calculateOffset4.y;
  ...
  var _ElementsPDF = new ElementsPDF(),
      addElementsToPDF = _ElementsPDF.addElementsToPDF;
  ...
  var PdfPrinter = /*#__PURE__*/function (_Control) {
    ...
    _createClass(PdfPrinter, [{
      ...
      value: function _printMap(form) {
        ...
        var dim = this._options.paperSizes.find(function (e) {
          return e.value === _this3._form.format;
        }).size;
